...
export default function PostCard({ post, delay = 0, featured = false }) {
  ...
  const color = post.coverColor || post.color || '#6366f1';
  ...
  return (
    ...
    >
      {/* Top accent */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: `linear-gradient(90deg, ${color}, ${color}44)`, borderRadius: '16px 16px 0 0' }} />

      {/* Glow orb */}
      <div style={{ position: 'absolute', top: '-30px', right: '-30px', width: '100px', height: '100px', borderRadius: '50%', background: `radial-gradient(circle, ${color}20 0%, transparent 70%)`, pointerEvents: 'none' }} />

      {/* Category + featured */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
        <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 700, color, background: `${color}18`, border: `1px solid ${color}30` }}>
          {post.category}
        </span>
        {post.featured && (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ color: '#4a5568', fontSize: '11px' }}>⏱️ {post.readTime}</span>
          <span style={{ color: '#4a5568', fontSize: '11px' }}>👁️ {(post.views || 0).toLocaleString()}</span>
        </div>
        <motion.div whileHover={{ x: 4 }} style={{ color, display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 600 }}>
          Read →
        </motion.div>
      </div>
    </motion.div>
